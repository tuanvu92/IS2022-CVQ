...
def convert_m4a_worker(pid, file_list, dst_dir):
    tqdm_text = "#" + "{}".format(pid).zfill(3)
    print(len(file_list))
    with tqdm(total=len(file_list), desc=tqdm_text, position=pid+1) as pbar:
        for fname in file_list:
            output_fname = join(dst_dir, "/".join(fname.split("/")[-3:])).replace(".m4a", ".wav")
...
def convert_m4a_multi_process(src_dir, dst_dir, n_process=12):
    print("Get list of files...")
    file_list = get_list_of_files(src_dir)
    file_list = [fname for fname in file_list if fname.find(".m4a") != -1]
    ...
    jobs = [pool.apply_async(convert_m4a_worker, args=(i, n, dst_dir))
...
if __name__ == "__main__":
    convert_m4a_multi_process("/home/messier/PycharmProjects/data/vox/vox2/test/aac/",
                              "/home/messier/PycharmProjects/data/vox/vox2/test/wav/")
